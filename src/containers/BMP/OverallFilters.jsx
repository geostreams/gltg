...
import { FUNDING_AGENCIES, FUNDING_YEAR_RANGES } from './config';
...
const OverallFilters = ({ setParameterString }) => {
    ...
    const programDialogText = (
        <>
            <p>
                <strong>EQIP:</strong> The Environmental Quality Incentives Program
        (EQIP) provides financial and technical assistance to agricultural
        producers to address natural resource concerns and deliver environmental
        benefits.
            </p>
            <br />
            <p>
                <strong>CSP:</strong> The Conservation Stewardship Program (CSP) helps
        agricultural producers maintain and improve their existing conservation
        systems and adopt additional conservation activities to address priority
        resource concerns.
            </p>
            <br />
            <p>
                <strong>EPA 319:</strong> The EPA Section 319 grant program supports a
        range of activities including technical assistance, financial
    ...
    const fundCostInfo = (
        <>
            <p>
                <strong>FUNDING:</strong> This option allows users to view the total
        funding allocated to various programs. It provides insights into the
        budgetary allocation and distribution across different sectors and
        activities.
            </p>
            <br />
            <p>
                <strong>PROGRAM COUNT:</strong> By selecting this option, users can view
        the total number of programs initiated or currently running. It's a
        metric to understand the volume and scale of efforts made in different
        areas.
            </p>
        </>
    );
    ...
    React.useEffect(() => {
        ...
        if (allYears) {
            urlEndPoint += `_bmp_${fundingAgency.toUpperCase()}_all_`;
        } else {
    ...
    }, [fundingAgency, selectedYear, selectedFunding, selectedStateBoundary, allYears]);
        ...
        <>
            ...
            Select Program Type
                    ...
                    <RadioGroup
                        aria-label="boundary"
                        name="boundary"
                        value={selectedStateBoundary.toString()}
                        onChange={(event) =>
                            setSelectedStateBoundary(event.target.value === 'true')}
                        row
                    >
                        <FormControlLabel
                            value="true"
            ...
            Select Funding/Practice Count
                    ...
                    <RadioGroup
                        ...
                        name="funding-type"
                        value={selectedFunding.toString()}
                        onChange={(event) =>
                            setSelectedFunding(event.target.value === 'true')}
                        row
                    >
                        <FormControlLabel
                            value="true"
