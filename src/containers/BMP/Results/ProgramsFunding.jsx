  ...
  props.data.forEach((d) => {
    ...
    if (tableData[boundaryId]) {
      tableData[boundaryId][d.program] =
        (tableData[boundaryId][d.program] || 0) + (d["funding-sum"] || 0);
    } else {
  ...
  return (
    <>
      <Typography variant="body1" paragraph>
        Distribution of the funding of conservation practices across selected
        area by year. Conservation practices are distinguished by funding
        source.
      </Typography>
      <VegaLite
                ...
                {programs.map((program: string) => (
                  <TableCell key={program} align="center">
                    {boundaryPrograms[program]
                      ? `$${precision(boundaryPrograms[program], 0)}`
                      : "-"}
                  </TableCell>
                ))}
              </TableRow>
