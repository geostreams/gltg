...
const BMP = ({ dispatch }: Props) => {
	...
	const [filters, dispatchFilterUpdate] = React.useReducer<
		Filters,
		FiltersAction,
	...
	const [hoveredBoundaryInfo, updateHoveredBoundaryInfo] = React.useState<
		[[string, string]],
	>([]);
	...
	React.useEffect(() => {
		if (hasConfig && config[filters.boundaryType]) {
			...
			if (previous.selectedBoundaries !== filters.selectedBoundaries) {
				...
				LAYERS[filters.boundaryType].setStyle((feature) =>
					getStyle(
						boundaryOptions,
						feature,
						BOUNDARIES[filters.boundaryType].layer.featureIdKey,
			...
			{hasConfig ? (
				...
				>
					...
					<Grid className={classes.mapContainer} item xs={6}>
						...
						>
							<BaseControlPortal
								el={mapControlsRef.current.boundaryInfo.element}
							>
								<Container>
									<Typography variant="subtitle2">
										Select boundaries using the map or the
										form on the right
									</Typography>
									<Divider />
									{hoveredBoundaryInfo.map(
										([label, value]) => (
											<Typography
												key={label}
												variant="body2"
					...
					<Grid className={classes.sidebar} item xs={6}>
						<Sidebar />
					</Grid>
				</Grid>
			) : null}
